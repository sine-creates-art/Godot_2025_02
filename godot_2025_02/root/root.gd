...
func _generate_main_menu() -> void:
	...
	var main_menu_app = SceneFactory.new_main_menu_app(main_menu_data);
	active_scene_parent.add_child(main_menu_app);
...
func _generate_game() -> void:
	...
	# Game
	var game_app: GameApp = SceneFactory.new_game_app(pause_menu_data);
	active_scene_parent.add_child(game_app);
	game_started.emit();
